  ...
  def spec = suite("Day12Part1")(
    ...
    testM("spread once") {
      ...
      for
        data       <- FileReader.getContent("input/day12/example1.txt")
        treeLine   <- TreeLine.fromString(data)
        testResult <- assertM(ZIO.succeed(treeLine.spread(1).toString))(equalTo(expected))
      yield testResult
    },
    testM("spread three times") {
      ...
      for
        data       <- FileReader.getContent("input/day12/example1.txt")
        treeLine   <- TreeLine.fromString(data)
        testResult <- assertM(ZIO.succeed(treeLine.spread(3).toString))(equalTo(expected))
      yield testResult
    },
    testM("spread 20 times") {
      ...
      for
        data       <- FileReader.getContent("input/day12/example1.txt")
        treeLine   <- TreeLine.fromString(data)
        testResult <- assertM(ZIO.succeed(treeLine.spread(20).value))(equalTo(expected))
      yield testResult
    },
